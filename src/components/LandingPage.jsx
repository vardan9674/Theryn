...
function CoachProduct({ screen = "dashboard", compact = false }) {
  ...
  return <div className={compact ? "coach-product compact" : "coach-product"}>
    ...
    <div ref={ref} className="coach-product-viewport" style={{height:height * width / base}} inert="" aria-hidden="true">
      <iframe key={screen} title="Theryn coach product with sample data" tabIndex={-1} loading="lazy" src={`/product-preview.html?screen=${screen}`} style={{width:base, height, transform:`scale(${width / base})`, transformOrigin:"top left", border:0}} />
    </div>
  </div>;
}
function FittedHero() {
