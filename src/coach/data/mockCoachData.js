...
import { generateToken, connectCode as newCode, linkClientData } from "../lib/clientLinks.js";
...
function makeState() {
  ...
  const clientLinks = { "manual:m1": { id: "lnk-m1", token: "previewRaviToken_abcdefghijklmnopqrstu", requested: ["chest", "waist", "hips", "arm", "thigh"], opens: 6, submissions: 2, created_at: daysAgo(10).toISOString(), connect_code: "K7QM2P", connect_tries: 0 } };
  const submissions = [
...
export function createMockCoachData() {
  ...
  return {
    ...
    async softDeleteTemplate(id) { await wait(200); st.templates = st.templates.filter((x) => x.id !== id); delete st.assignments[id]; for (const m of st.manual) if (planTemplate(m.plan)?.id === id) m.plan = stampTemplate(m.plan, null); },
    ...
    async updateSubmission(id, payload) { await wait(150); const s0 = st.submissions.find((x) => x.id === id); if (!s0) throw new Error("That workout is gone."); s0.payload = payload; notify(); },
